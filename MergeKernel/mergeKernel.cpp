      ...
      for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; ++I) {
        if (CallInst *CI = dyn_cast<CallInst>(&*I)) {
          ...
          if (calledFunc->getName().contains("llvm.nvvm.barrier")) {
            ...
            llvm::CloneFunctionInto(newFunc, deviceKernel, VMap, false,
                                    Returns);
            errs() << *(newFunc) << "\n";
            ...
            for (inst_iterator I = inst_begin(newFunc), E = inst_end(newFunc);
              ...
              if (nvvmCall2arg.find(calledFuncName) == nvvmCall2arg.end())
                continue;
            ...
            auto newLd = new LoadInst(
                cast<PointerType>(originalAlloc->getType())->getElementType(),
