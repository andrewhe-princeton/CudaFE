...
public:
  ...
  std::map<Value *, int> dim2classify; // 0: no meaning 1:outermost loop of grid
                                       // 2:outermost loop of block
  int gridLoopCnt = 0;
  int blockLoopCnt = 0;
...
struct MergeKernel : public ModulePass {
  ...
  void findThreadDim(KernelProfile *kernelProfile, Function &F,
                     LoadInst *DimArg, bool isBlockDim) {
    // LoadInst *DimArg = dyn_cast<LoadInst>(CI->getArgOperand(2));
    assert(DimArg && "mergeKernel: DimArg is not a load inst\n");
    GetElementPtrInst *ArgGep =
        dyn_cast<GetElementPtrInst>(DimArg->getOperand(0));
    ...
    for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
      ...
      if (!bitcast)
        continue;
      if (bitcast->getOperand(0) != Coerce)
        continue;
      CoerceBitCast = bitcast;
      break;
    }
    assert(CoerceBitCast &&
           "mergeKernel: CoerceBitCast is not a bit cast inst\n");
    ...
    for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
      ...
      if (CI->getCalledFunction()->getName() != "llvm.memcpy.p0i8.p0i8.i64")
        continue;
      if (CI->getArgOperand(0) != CoerceBitCast)
        continue;
    ...
    for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
      ...
      if (!CI)
        continue;
      if (!CI->getCalledFunction()->getName().contains("_ZN4dim3C2Ejjj"))
        continue;
      if (CI->getArgOperand(0) != AggAlloca)
        continue;
    ...
    if (!foundDim) {
      ...
      for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
        ...
        if (!bitcast)
          continue;
        if (bitcast->getOperand(0) != AggAlloca)
          continue;
        AggBitCast2 = bitcast;
        break;
      }
      ...
      for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
        ...
        if (CI->getCalledFunction()->getName() != "llvm.memcpy.p0i8.p0i8.i64")
          continue;
        if (CI->getArgOperand(0) != AggBitCast2)
          continue;
        BitCastInst *bitcast = dyn_cast<BitCastInst>(CI->getArgOperand(1));
        DimBitCast = bitcast;
        break;
      ...
      for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
        ...
        if (!CI->getCalledFunction()->getName().contains("_ZN4dim3C2Ejjj"))
          continue;
        if (CI->getArgOperand(0) != DimAlloca)
          continue;
        for (int i = 1; i <= 3; ++i) {
          Value *dim = CI->getArgOperand(i);
          // if(ConstantInt *dimConst = dyn_cast<ConstantInt>(dim))
  ...
  bool runOnModule(Module &M) override {
    ...
    for (Module::iterator FI = M.begin(), FE = M.end(); FI != FE; ++FI) {
        ...
        if (CallInst *CI = dyn_cast<CallInst>(&*I)) {
          ...
          if (calledFunc->getName().contains("llvm.nvvm.barrier")) {
            ...
            Function *F = Function::Create(
                sqrtFuncTy, Function::ExternalLinkage, "fmax", M);
            auto *CB = llvm::dyn_cast<llvm::CallBase>(CI);
            SmallVector<Value *, 4> args(CB->arg_begin(), CB->arg_end());

            auto sqrtFunc =
                F->getParent()->getOrInsertFunction("fmax", sqrtFuncTy);
            auto sqrtCall = CallInst::Create(sqrtFunc, args, "", CI);
            for (User *U : CI->users()) {
              Instruction *inst = dyn_cast<Instruction>(U);
              if (!inst)
                continue;
              for (auto OI = inst->op_begin(), OE = inst->op_end(); OI != OE;
                   ++OI) {
                Value *val = *OI;
                if (val == CI)
                  *OI = sqrtCall;
            ...
            auto sqrtFuncTy = calledFunc->getFunctionType();
            Function *F = Function::Create(
                sqrtFuncTy, Function::ExternalLinkage, "sqrt", M);
            auto *CB = llvm::dyn_cast<llvm::CallBase>(CI);
            SmallVector<Value *, 4> args(CB->arg_begin(), CB->arg_end());

            auto sqrtFunc =
                F->getParent()->getOrInsertFunction("sqrt", sqrtFuncTy);
            auto sqrtCall = CallInst::Create(sqrtFunc, args, "", CI);
            for (User *U : CI->users()) {
              Instruction *inst = dyn_cast<Instruction>(U);
              if (!inst)
            ...
            for (int i = 1; i <= 3; ++i) {
              ...
              auto gep = GetElementPtrInst::Create(
                  dimPtrTy->getPointerElementType(), dimPtr, idxList,
                  "dim3gep." + std::to_string(i - 1), CI);
            ...
            BranchInst *CF2Remove =
                dyn_cast<BranchInst>(CI->getParent()->getTerminator());
            ...
            if (cmp) {
              if (cmp->getPredicate() == CmpInst::ICMP_EQ ||
                ...
                if (ConstantInt *integer = dyn_cast<ConstantInt>(opnd0)) {
                  ...
                  else if (integer->getZExtValue() == 0 && opnd1 == CI &&
                           cmp->getPredicate() == CmpInst::ICMP_NE)
                    ConfigureThenBranchPattern = true;
                } else if (ConstantInt *integer =
                               dyn_cast<ConstantInt>(opnd1)) {
                  if (integer->getZExtValue() == 0 && opnd0 == CI &&
                      cmp->getPredicate() == CmpInst::ICMP_EQ)
                    ConfigureThenBranchPattern = true;
                  else if (integer->getZExtValue() == 0 && opnd0 == CI &&
                           cmp->getPredicate() == CmpInst::ICMP_NE)
                    ConfigureThenBranchPattern = false;
                }
                if (ConfigureThenBranchPattern) {
                  CF2Remove->setCondition(ConstantInt::get(
                      Type::getInt1Ty(CF2Remove->getContext()), 1));
                  kernelProfiles[CI]->kernelBB = CF2Remove->getSuccessor(0);
                  errs() << "mergeKernel: kernelBB: "
                         << *(kernelProfiles[CI]->kernelBB) << "\n";
                } else {
                  CF2Remove->setCondition(ConstantInt::get(
                      Type::getInt1Ty(CF2Remove->getContext()), 0));
            ...
            // TODO: mem2reg sort of changes everything with the pattern
            // matching
            ...
            if (isa<ConstantInt>(CI->getArgOperand(0))) {
              ...
              if (ConstantInt *constInt = dyn_cast<ConstantInt>(dim))
                if (constInt->getSExtValue() == 1)
                  isOne = true;
              if (!isOne)
                kernelProfiles[CI]->gridLoopCnt++;
            } else {
              findThreadDim(kernelProfiles[CI], *F,
                            dyn_cast<LoadInst>(CI->getArgOperand(0)), false);
            }
            ...
            if (isa<ConstantInt>(CI->getArgOperand(2))) {
              ...
              if (ConstantInt *constInt = dyn_cast<ConstantInt>(dim))
                if (constInt->getSExtValue() == 1)
                  isOne = true;
              if (!isOne)
                kernelProfiles[CI]->blockLoopCnt++;
            } else {
              findThreadDim(kernelProfiles[CI], *F,
                            dyn_cast<LoadInst>(CI->getArgOperand(2)), true);
            }
            ...
            if (kernelName == "")
              kernelName = host_kernelName;
            errs() << "mergeKernel: found kernelName: " << kernelName << "\n";
            errs() << "host kernel name: " << host_kernelName << '\n';
            ...
            for (Module::iterator FI = M->begin(), FE = M->end(); FI != FE;
              ...
              errs() << "Function names: " << funcName << '\n';
              if (funcName.contains(kernelName) &&
                  funcName != host_kernelName) {

                errs() << "Device Kernel assigned: \n";
            ...
            for (auto arg = deviceKernel->arg_begin();
                 arg != deviceKernel->arg_end(); ++arg)
              argTys.push_back(arg->getType());
            for (int i = 0; i < 12; i++)
              argTys.push_back(i32Ty);
            FunctionType *funcTy =
                FunctionType::get(deviceKernel->getReturnType(), // return type
                                  ArrayRef<Type *>(argTys),      // arg types;
                                  false);
            kernelProfiles[CI]->newFunc = Function::Create(
                funcTy, deviceKernel->getLinkage(), deviceKernel->getName(),
            ...
            // for(auto arg = deviceKernel->arg_begin(); arg !=
            // deviceKernel->arg_end(); ++arg) {
            ...
            int i = 0;
            for (auto arg = newFunc->arg_begin(); arg != newFunc->arg_end();
                 ++arg) {
            ...
            for (inst_iterator I = inst_begin(newFunc), E = inst_end(newFunc);
              ...
              std::string funcNameStr = calledFuncName.str();
              if (nvvmCall2arg.find(funcNameStr) == nvvmCall2arg.end())
                continue;
              auto argName = nvvmCall2arg[funcNameStr];
              ...
              for (auto arg = newFunc->arg_begin(); arg != newFunc->arg_end();
                ...
                for (User *U : ci->users()) {
                  ...
                  if (!inst)
                    continue;
                  for (auto OI = inst->op_begin(), OE = inst->op_end();
                       OI != OE; ++OI) {
                    Value *val = *OI;
                    if (val == ci)
                      *OI = arg;
            ...
            for (auto I : cudaCall2remove) {
              ...
              errs() << "mergeKernel: what does I belong to?"
                     << I->getParent()->getName();
              I->eraseFromParent();
            }
            insts2Remove.push_back(CI);
          } else if (calledFunc->getName().contains("cudaMalloc") &&
                     calledFunc->getName() != "cudaMalloc") {
            ...
            // PointerType* points2Ty =
            // dyn_cast<PointerType>(Ty->getPointerElementType()); Instruction*
            // Malloc = CallInst::CreateMalloc(CI,
            //                                    AllocSize->getType(),
            //                                    points2Ty->getPointerElementType(),
            ...
            // ++I) {
            //     for (Instruction::op_iterator I_Op = (&*I)->op_begin(), E_Op
            //     = (&*I)->op_end(); I_Op != E_Op; ++I_Op){
            //       if(*I_Op == devDataPtr){
            //         *I_Op = Malloc;
            //       }
            //     }
            ...
            mode->isOne()
                ? originalBitcast = dyn_cast<BitCastInst>(CI->getArgOperand(1))
                : originalBitcast = dyn_cast<BitCastInst>(CI->getArgOperand(0));
            Instruction *devBitcast = nullptr;
            mode->isOne()
                ? devBitcast = dyn_cast<BitCastInst>(CI->getArgOperand(0))
                : devBitcast = dyn_cast<BitCastInst>(CI->getArgOperand(1));
            assert(originalBitcast && devBitcast &&
                   "mergeKernel: didn't find bitcast from cudaMemcpy!\n");
            LoadInst *devLd = dyn_cast<LoadInst>(devBitcast->getOperand(0));
            LoadInst *originalLd =
                dyn_cast<LoadInst>(originalBitcast->getOperand(0));
            assert(originalLd && devLd &&
                   "mergeKernel: didn't find load from cudaMemcpy!\n");
            AllocaInst *devAlloc = dyn_cast<AllocaInst>(devLd->getOperand(0));
            AllocaInst *originalAlloc =
                dyn_cast<AllocaInst>(originalLd->getOperand(0));
            assert(devAlloc && originalAlloc &&
                   "mergeKernel: didn't find alloca from cudaMemcpy!\n");
            std::map<AllocaInst *, LoadInst> originalAlloc2newLD;
            auto newLd =
                new LoadInst(cast<PointerType>(originalAlloc->getType())
                                 ->getPointerElementType(),
                             originalAlloc, "ldHost", originalLd);
            ...
            for (auto user : originalAlloc->users()) {
              ...
              if (StoreInst *st = dyn_cast<StoreInst>(user)) {
                errs() << "mergeKernel: found originalAlloc store:" << *st
                       << "\n";
                if (BitCastInst *cast =
                        dyn_cast<BitCastInst>(st->getOperand(0))) {
                  ...
                  if (CallInst *ci = dyn_cast<CallInst>(cast->getOperand(0))) {
                    errs() << "mergeKernel: found originalAlloc ci:" << *ci
                           << "\n";
                    if (!ci->getCalledFunction()->getName().contains("malloc"))
                      continue;
                    cpySize = ci->getArgOperand(0);
                    LLVMContext &C = cpySize->getContext();
                    MDNode *mdSize = nullptr;
                    if (Instruction *sizeInst =
                            dyn_cast<Instruction>(cpySize)) {
                      std::string mdDataSize = "tulip.target.datasize";
                      mdSize = MDNode::get(
                          C, MDString::get(C, std::to_string(mdID)));
                    ...
                    mdSize ? N = MDNode::get(C, mdSize)
                           : N = MDNode::get(
                                 C, ValueAsMetadata::get(
                                        dyn_cast<ConstantInt>(cpySize)));
                    mode->isOne()
                        ? ci->setMetadata("tulip.target.mapdata.to", N)
            ...
            for (auto user : devAlloc->users()) {
              if (LoadInst *ld = dyn_cast<LoadInst>(user)) {
                for (auto user : ld->users()) {
                  ...
                  for (auto OI = UI->op_begin(), OE = UI->op_end(); OI != OE;
                       ++OI) {
                    Value *val = *OI;
                    if (val == ld)
                      *OI = newLd;
            ...
            // TODO: better to create an empty function that doesn't really do
            // anything
            if (mode->isOne()) {
            ...
            // argTyVec.push_back(PointerType::get(Type::getInt8Ty(F->getContext()),
            // 0));
            // argTyVec.push_back(PointerType::get(Type::getInt8Ty(F->getContext()),
            // 0)); argTyVec.push_back(Type::getInt64Ty(F->getContext()));
            ...
            // auto MemCpyFunc =
            // F->getParent()->getOrInsertFunction("llvm.memcpy.p0i8.p0i8.i64",
            // memcpyFuncTy); CallInst *NewCI = CallInst::Create(MemCpyFunc,
            // args, "", CI);
            ...
            // :
            //                dest =
            //                dyn_cast<Instruction>(CI->getArgOperand(1));
            ...
            // mdSize ? N = MDNode::get(C, mdSize) :
            //          N = MDNode::get(C,
            //          ValueAsMetadata::get(dyn_cast<ConstantInt>(cpySize)));
            // mode->isOne() ? dest->setMetadata("tulip.target.mapdata.to", N) :
            //                 dest->setMetadata("tulip.target.mapdata.from",
            //                 N);
      ...
      for (auto [configureCI, kernelProfile] : kernelProfiles) {
        ...
        if (kernelProfile->gridLoopCnt + kernelProfile->blockLoopCnt != 0) {
          ...
          std::vector<BasicBlock *> succs(succ_begin(kernelBB),
                                          succ_end(kernelBB));
          ...
          assert(kernelPred &&
                 "mergeKernel: kernel has multiple predecessors\n");
          ...
          for (auto itNum : loopDims) {
            ...
            auto header = BasicBlock::Create(
                kernelBB->getContext(), "header." + std::to_string(loopCnt), F,
                kernelBB);
          ...
          for (int i = loopCnt - 1; i >= 0; --i) {
            ...
            auto latch =
                BasicBlock::Create(kernelBB->getContext(),
                                   "latch." + std::to_string(i), F, kernelBB);
            if (!lastLatch)
              lastLatch = latch;
          ...
          while (!headerNests2.empty()) {
            ...
            headerNests2.pop();
            auto nextHeader =
                headerNests2.empty() ? nullptr : headerNests2.top();

            // create phi node
            Type *phiTy = header2itNum[header]->getType();
            auto indvar = PHINode::Create(
                phiTy, 2, "indvar." + std::to_string(i), header);
            indvars.push_back(indvar);
            CmpInst *cmp = CmpInst::Create(
                Instruction::ICmp, CmpInst::ICMP_ULT, indvar,
                header2itNum[header], "exitCheck." + std::to_string(i), header);
            BranchInst *term = nullptr;

            // create increment in latch
            auto latch = header2latch[header];
            Value *incr = BinaryOperator::Create(
                Instruction::Add, indvar, ConstantInt::get(phiTy, 1),
            ...
            } else if (!nextHeader || !nextHeader->hasName() ||
                       nextHeader->getName() == "") {
              term = BranchInst::Create(prevHeader, loopExit, cmp, header);
              indvar->addIncoming(ConstantInt::get(phiTy, 0), kernelPred);
            } else if (!prevHeader) {
              term = BranchInst::Create(kernelBB, header2latch[nextHeader], cmp,
                                        header);
              indvar->addIncoming(ConstantInt::get(phiTy, 0), nextHeader);
            } else {
              term = BranchInst::Create(prevHeader, header2latch[nextHeader],
                                        cmp, header);
              indvar->addIncoming(ConstantInt::get(phiTy, 0), nextHeader);
            }
            ...
            if (kernelProfile->dim2classify[header2itNum[header]] == 1 ||
              ...
              MDNode *N = MDNode::get(C, MDString::get(C, ""));
              if (kernelProfile->dim2classify[header2itNum[header]] == 1) {
                if (kernelProfile->gridLoopCnt > 1)
                  term->setMetadata("tulip.doall.loop.grid.collapse", N);
                else
                  term->setMetadata("tulip.doall.loop.grid", N);
              }
              if (kernelProfile->dim2classify[header2itNum[header]] == 2) {
                if (kernelProfile->blockLoopCnt > 1)
                  term->setMetadata("tulip.doall.loop.block.collapse", N);
                else
        ...
        if (loopDims.size() == 2) {
          ...
          if (!dim->getType()->isIntegerTy(32))
            arg = CastInst::CreateTruncOrBitCast(dim, i32Ty, "dim.cast",
                                                 kernelCall);
          ...
          if (!arg->getType()->isIntegerTy(32))
            arg = CastInst::CreateTruncOrBitCast(indvar, i32Ty, "dim.cast",
                                                 kernelCall);
          ...
          if (!arg->getType()->isIntegerTy(32))
            arg = CastInst::CreateTruncOrBitCast(indvar, i32Ty, "dim.cast",
                                                 kernelCall);
          indvarsExtended.push_back(arg);
          indvarsExtended.push_back(
              ConstantInt::get(Type::getInt32Ty(kernelCall->getContext()), 0));
          indvarsExtended.push_back(
              ConstantInt::get(Type::getInt32Ty(kernelCall->getContext()), 0));
        } else {
          for (auto itNum : loopDims) {
            ...
            if (ConstantInt *constInt = dyn_cast<ConstantInt>(itNum))
              if (constInt->getSExtValue() == 1) {
                indvarsExtended.push_back(ConstantInt::get(
                    Type::getInt32Ty(kernelCall->getContext()), 0));
                continue;
              }
        ...
        CallInst *newKernelCall =
            CallInst::Create(newFunc->getFunctionType(),       // function type
                             newFunc,                          // function
    ...
    for (Module::global_iterator I = M.global_begin(), E = M.global_end();
      ...
      if (!globVal->hasInitializer())
        continue;
      if (globVal->getAddressSpace() != 3)
        continue;
    ...
    for (auto [func, sharedGlobs] : func2SharedGlobs) {
      ...
      for (auto glob : sharedGlobs) {
        ...
        AllocaInst *sharedAlloc =
            new AllocaInst(pointedTy, 0, nullptr,
                           "sharedMem" + std::to_string(i), &(entryBB.front()));
        ++i;
        for (auto globInst : glob2Insts[glob]) {
          if (globInst->getParent()->getParent() != func)
            continue;
        ...
        for (auto [LI, gepOp] : inst2gepOp) {
          if (LI->getParent()->getParent() != func)
            continue;
          std::vector<Value *> idxList(gepOp->idx_begin(), gepOp->idx_end());
          auto gepInst = GetElementPtrInst::Create(
              gepOp->getSourceElementType(), sharedAlloc, makeArrayRef(idxList),
              "sharedMem.gep" + std::to_string(i), LI);
    ...
    for (auto [func, sharedMemVars] : func2SharedMems) {
      errs() << "mergeKernel: func that contain shared mem objects: "
             << func->getName() << "\n";
...
static RegisterPass<MergeKernel> X("merge-kernel",
                                   "merge cuda kernel back to main file",
